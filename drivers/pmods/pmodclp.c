...
void write_display_control_cmd(struct pmodclp_device *dev)
{
 ...
 */
int gpio_par_define_data_direction(struct par_device *par_dev, bool fOutput, unsigned char bOutputVal)
{
	int i;
	int status = 0;
 ...
 */
unsigned char gpio_par_read_byte(struct par_device *par_dev)
{
	int i;
 ...
 */
void gpio_par_write_byte(struct par_device *par_dev, unsigned char bData)
{
	int i;
	// Clear RW
	gpio_set_value(par_dev->iRW, 0);
 ...
 */
unsigned char pmodclp_read_status(struct par_device *par_dev)
{
 ...
 */
void pmodclp_wait_until_not_busy(struct par_device *par_dev)
{
	unsigned char bStatus;
 ...
 */
void pmodclp_write_data(struct par_device *par_dev, char *txt_buf, int cnt)
{
	int	i;
	// set RS, meaning data
 ...
 */
void pmodclp_init(struct par_device *par_dev)
{
...
int parse_cmd(char c, unsigned char *pars, int idx_par, int par_type, struct pmodclp_device *dev)
{
...
void parse_text(char *txt_buf, int cnt, struct pmodclp_device *dev)
{
		...
		else
		{
			if(is_cmd)
			{
				...
				if(!is_consumed_char)
				{
					...
					printk(KERN_INFO " Wrong command: %.*s \n", (parse_ptr - processed_ptr + 1), processed_ptr);
					processed_ptr = parse_ptr;
				}	
	...
	if(((parse_ptr - processed_ptr) > 0))
	{
		...
		{
			printk(KERN_INFO " Wrong command: %.*s \n", (parse_ptr - processed_ptr + 2), processed_ptr + 1);
		}
	}
...
struct file_operations pmodclp_cdev_fops = {
	.owner = THIS_MODULE,
	.write = pmodclp_write,
	.read = pmodclp_read,
	.open = pmodclp_open,
...
static int pmodclp_setup_cdev(struct pmodclp_device *dev, dev_t *dev_id) 
{
	...
	*dev_id = MKDEV(MAJOR(pmodclp_dev_id), cur_minor++);
	...
	if(IS_ERR(device)) {
		...
		printk(KERN_INFO  "failed to create device node %s, err %d\n",
				dev->name, status);
		cdev_del(&dev->cdev);
	}
...
static const struct of_device_id pmodclp_of_match[] __devinitconst = {
 ...
 */
static int __devinit pmodclp_of_probe(struct platform_device *pdev)
{
	...
	pmodclp_dev = (struct pmodclp_device*) kzalloc(sizeof(*pmodclp_dev), GFP_KERNEL);
	if(!pmodclp_dev) {
		status = -ENOMEM;
		goto dev_alloc_err;
	}
	...
	pmodclp_dev->txt_buf = (unsigned char*) kmalloc(TXT_BUF_SIZE, GFP_KERNEL);
	...
	printk(KERN_INFO DRIVER_NAME " %s: iRS: 0x%lx\n", np->name, pmodclp_dev->par_dev.iRS);
	printk(KERN_INFO DRIVER_NAME " %s: iRW: 0x%lx\n", np->name, pmodclp_dev->par_dev.iRW);
	printk(KERN_INFO DRIVER_NAME " %s: iE  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iE);
	printk(KERN_INFO DRIVER_NAME " %s: iBK  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iBK);

	printk(KERN_INFO DRIVER_NAME " %s: iData[0]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[0]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[1]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[1]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[2]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[2]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[3]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[3]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[4]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[4]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[5]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[5]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[6]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[6]);
	printk(KERN_INFO DRIVER_NAME " %s: iData[7]  : 0x%lx\n", np->name, pmodclp_dev->par_dev.iData[7]);
#endif
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : pmodclp device class registered.\n");
#endif
	}
	...
	if (status) {
		printk(KERN_INFO " pmodclp_probe: Error adding %s device: %d\n", DRIVER_NAME, status);	
		goto cdev_add_err;
	}
	...
	if (status) {
		printk(KERN_INFO " spi_probe: Error init gpio: %d\n", status);	
		goto cdev_add_err;
	}
...
cdev_add_err:
	...
	printk(KERN_INFO DRIVER_NAME " Free text buffer.\n");
...
static int pmodclp_init_gpio(struct pmodclp_device *pmodclp_dev) 
{
	...
	for (i = 0; i < array_size; i++) {
		...
		if(!status) {
			printk(KERN_INFO "!! gpio_is_valid for GPIO %d, %s FAILED!, status: %d\n",
					pmodclp_ctrl[i].gpio, pmodclp_ctrl[i].label, status);
			goto gpio_invalid;
		}
	}
	
	printk(KERN_INFO "** gpio_request_array array_size = %d, ARRAY_SIZE = %d\n", array_size, ARRAY_SIZE(pmodclp_ctrl));

	status = gpio_request_array(pmodclp_ctrl, ARRAY_SIZE(pmodclp_ctrl));
	if(status) {
		printk(KERN_INFO "!!  gpio_request_array FAILED!\n");
		printk(KERN_INFO "          status is: %d, array_size = %d, ARRAY_SIZE = %d\n", status, array_size, ARRAY_SIZE(pmodclp_ctrl));
...
static int pmodclp_of_remove(struct platform_device *pdev) 
{
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " %s : Free text buffer.\n", np->name);
#endif
	...
	if(device_num == 0) {
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : Destroy pmodclp_gpio Class.\n");
#endif
...
static struct platform_driver pmodclp_driver = {
	...
	.remove = __devexit_p(pmodclp_of_remove),
};
