...
struct file_operations pmodad1_cdev_fops = {
	.owner = THIS_MODULE,
	.open = pmodad1_open,
	.read = pmodad1_read,
};
 ...
 */
static int __init add_pmodad1_device_to_bus(struct pmodad1_device* dev) {
...
static int pmodad1_spi_probe(struct spi_device *spi) {
	...
	if (spi->master->flags & SPI_MASTER_HALF_DUPLEX) {
		status = -EINVAL;
		printk(KERN_INFO SPI_DRIVER_NAME "SPI settings incorrect: %d\n", status);
		goto spi_err;
	}
 ...
 */
static int __devexit pmodad1_spi_remove(struct spi_device *spi) 
{
	int status;
	struct pmodad1_device *dev;
...
static struct spi_driver pmodad1_spi_driver = {
	...
	.remove = __devexit_p(pmodad1_spi_remove),
};

static const struct of_device_id pmodad1_of_match[] __devinitconst = {
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : Destroy pmodad1 Class.\n");
#endif
...
static struct platform_driver pmodad1_driver = {
	...
	.remove = __devexit_p(pmodad1_of_remove),
};
