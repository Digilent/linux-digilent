/*
 * Digilent PmodDA1 linux driver for Zed
 */
...
#define PMODDA1_DEV_NUM 	2	// only 2 channels as SPI does not allow write on Data In line.
 ...
 */
int buf_sz = DEFAULT_BUF_SZ;
module_param(buf_sz, int, 0);

dev_t pmodda1_first_dev_id = 0;
struct spi_device *spi_device;
static unsigned int spi_drv_registered = 0;
static struct class *pmodda1_class = NULL;
...
struct pmodda1_device {
	...
	/* Data Buffer */
	unsigned char	*buf;	// Device data buffer
	/* Pin Assignment */
...
int write_spi_16(struct spi_device *spi, uint16_t cmd_data);
...
static ssize_t	pmodda1_write(struct file *fp, const char __user *buffer, size_t length, loff_t *offset) 
{
	...
	switch(minor_id){
		case 0: dac1.a_val = dev->buf[i-1];
			break;
		case 1: dac1.b_val = dev->buf[i-1];
			break;
	}
quit_write:
	mutex_unlock(mutex);
...
static ssize_t	pmodda1_read(struct file *fp, char __user *buffer, size_t length, loff_t *offset)
{
	...
	if (buffer == NULL) {
		dev_err(&dev->spi->dev, "da1_read: ERROR: invalid buffer address: 0x%08X\n", (unsigned int)buffer);
		retval = -EINVAL;
		goto quit_read;
	}
	...
	switch (minor_id){
		case 0: rd_val = dac1.a_val;
			break;
		case 1: rd_val = dac1.b_val;
			break;
		default:
...
printk(KERN_INFO DRIVER_NAME "Read values Last Value\t%X\n", rd_val);
	for (i = 0; i < cnt; i++) 
		dev->buf[i] = rd_val;
	
	retval = copy_to_user((void *)buffer, (void *)dev->buf, cnt);
	if (!retval)
...
struct file_operations pmodda1_cdev_fops = {
	.owner = THIS_MODULE,
	.write = pmodda1_write,
	.read = pmodda1_read,
	.open = pmodda1_open,
 ...
 */
static int __init add_pmodda1_device_to_bus(struct pmodda1_device* dev) {
	...
	put_device(&spi_master->dev);
	printk(KERN_INFO DRIVER_NAME " SPI initialized, max_speed_hz\t%d\n", spi_device->max_speed_hz);	
...
static int pmodda1_setup_cdev(struct pmodda1_device *dev, dev_t *dev_id, int idx, struct spi_device *spi) 
{
	...
	*dev_id = MKDEV(major_id, minor_id);
	...
	printk(KERN_INFO SPI_DRIVER_NAME "pmodda1_setup_cdev: Create device %s, major %d, minor %d\n",
...
static int pmodda1_spi_probe(struct spi_device *spi) {
	...
	for (i = 0; i < PMODDA1_DEV_NUM; i++) {
		rgpmodda1_devices[i]->buf = (unsigned char *)kmalloc(buf_sz, GFP_KERNEL);
		if (!rgpmodda1_devices[i]->buf) {
			status = -ENOMEM;
			dev_err(&spi->dev, "Device value buffer allocation "
						"failed: %d\n", status);
			goto buf_alloc_err;
		}
...
buf_alloc_err:
	for (i = 0; i < PMODDA1_DEV_NUM; i++) {
		if(rgpmodda1_devices[i] && rgpmodda1_devices[i]->buf)
 ...
 */
static int __devexit pmodda1_spi_remove(struct spi_device *spi) 
{
	int status;
	struct pmodda1_device *dev;
...
static struct spi_driver pmodda1_spi_driver = {
	...
	.remove = __devexit_p(pmodda1_spi_remove),
};

static const struct of_device_id pmodda1_of_match[] __devinitconst = {
 ...
 */
static int __devinit pmodda1_of_probe(struct platform_device *pdev)
{
	struct pmodda1_device *pmodda1_dev;
	...
	pmodda1_dev = (struct pmodda1_device*) kzalloc(sizeof(*pmodda1_dev), GFP_KERNEL);
	if(!pmodda1_dev) {
		status = -ENOMEM;
		goto dev_alloc_err;
	}

	pmodda1_dev->buf = (unsigned char *)kmalloc(buf_sz, GFP_KERNEL);
	if (!pmodda1_dev->buf) {
		status = -ENOMEM;
		printk(KERN_INFO DRIVER_NAME "Device value buffer allocation "
					"failed: %d\n", status);
		goto buf_alloc_err;
	}
	...
	pmodda1_pdata = (struct spi_gpio_platform_data*) kzalloc(sizeof(*pmodda1_pdata), GFP_KERNEL);
	if(!pmodda1_pdata) {
		status = -ENOMEM;
	...
	/* Alloc Space for platform data structure */
	pmodda1_pdev = (struct platform_device*) kzalloc(sizeof(*pmodda1_pdev), GFP_KERNEL);
	if(!pmodda1_pdev) {
		status = -ENOMEM;
		goto pdev_alloc_err;
	}
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : pmodda1 device class registered.\n");
#endif
	}
...
#ifdef CONFIG_PMODS_DEBUG
	printk(KERN_INFO DRIVER_NAME " : Destroy pmodda1_gpio Class.\n");
#endif
...
static struct platform_driver pmodda1_driver = {
	...
	.remove = __devexit_p(pmodda1_of_remove),
};
