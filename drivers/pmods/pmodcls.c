...
static ssize_t	pmodcls_write(struct file *fp, const char __user *buffer, size_t length, loff_t *offset) 
{
	...
	if (status) {
		dev_err(&dev->spi->dev, "cls_write: "
				"Error writing text to SPI device\n");
		retval = -EFAULT;
		goto quit_write;
	}
...
static int parse_text(char *txt_buf, int cnt, struct pmodcls_device *dev)
{
	...
	while((!status) && (parse_buf < (txt_buf + cnt)))
	{
		...
		else
		{
			if(is_cmd)
			{
				if((*parse_buf) >= '0' && (*parse_buf) <= '9')
				{	// numeric char, build parameters values
					if(is_par1)
					{
 ...
 *
 *
 * This function breaks sends the string to the PmodCLS device over the SPI. It breaks the input string in chunks of 3 bytes in order to reduce the load on the receiving  
 * PmodCLS.
 * 
 */
static int txt_buf_to_display(char *txt_buf, int cnt, struct pmodcls_device *dev)
...
struct file_operations pmodcls_cdev_fops = {
	.owner = THIS_MODULE,
	.write = pmodcls_write,
	.read = pmodcls_read,
	.open = pmodcls_open,
 ...
 */
static int __init add_pmodcls_device_to_bus(struct pmodcls_device* dev) {
...
static int pmodcls_spi_probe(struct spi_device *spi) {
	...
	if (status) {
		printk(KERN_INFO " spi_probe: Error adding %s device: %d\n", SPI_DRIVER_NAME, status);	
...
static int __devexit pmodcls_spi_remove(struct spi_device *spi) 
{
	int status;
	struct pmodcls_device *dev;
...
static struct spi_driver pmodcls_spi_driver = {
	...
	.remove = __devexit_p(pmodcls_spi_remove),
};

static const struct of_device_id pmodcls_of_match[] __devinitconst = {
 ...
 */
static int __devinit pmodcls_of_probe(struct platform_device *pdev)
{
	struct pmodcls_device *pmodcls_dev;
	...
	pmodcls_dev = (struct pmodcls_device*) kzalloc(sizeof(*pmodcls_dev), GFP_KERNEL);
	if(!pmodcls_dev) {
		status = -ENOMEM;
		goto dev_alloc_err;
	}
	...
	pmodcls_dev->txt_buf = (uint8_t*) kmalloc(TXT_BUF_SIZE, GFP_KERNEL);
	...
	pmodcls_pdata = (struct spi_gpio_platform_data*) kzalloc(sizeof(*pmodcls_pdata), GFP_KERNEL);
	if(!pmodcls_pdata) {
		status = -ENOMEM;
	...
	/* Alloc Space for platform data structure */
	pmodcls_pdev = (struct platform_device*) kzalloc(sizeof(*pmodcls_pdev), GFP_KERNEL);
	if(!pmodcls_pdev) {
		status = -ENOMEM;
		goto pdev_alloc_err;
	}
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : pmodcls device class registered.\n");
#endif
	}
...
#ifdef CONFIG_PMODS_DEBUG
		printk(KERN_INFO DRIVER_NAME " : Destroy pmodcls_gpio Class.\n");
#endif
...
static struct platform_driver pmodcls_driver = {
	...
	.remove = __devexit_p(pmodcls_of_remove),
};
